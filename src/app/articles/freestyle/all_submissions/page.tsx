...
const Page = () => {
  ...
  const getAllSubmissions = async () => {
    const res = (
      await axios.get(
        `${process.env.NEXT_PUBLIC_APP_URL}/api/article_statement/freestyle/all_submissions`,
      )
    ).data;
  ...
  useEffect(() => {
    const fn = async () => {
      setSubmissions((await getAllSubmissions()).reverse());
    };
    fn();
  }, []);
      ...
      <Container>
        ...
        <div className="flex flex-col gap-4">
          {submissions.map((submission) => (
            <Link
              href={`/articles/freestyle/${submission.id}`}
              key={submission.id}
            >
